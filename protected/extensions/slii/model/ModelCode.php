...
class ModelCode extends CCodeModel
{
	...
	public $buildRelations=false;
	public $guessRelations=true;
	...
	public function rules()
	{
		return array_merge(parent::rules(), array(
			...
			array('tablePrefix, modelPath, baseClass, buildRelations, guessRelations', 'sticky'),
		));
	}
	...
	public function attributeLabels()
	{
		return array_merge(parent::attributeLabels(), array(
			...
			'guessRelations'=>'Guess Relations',
		));
	}
	...
	public function prepare()
	{
        ...
        if($this->guessRelations)
        {
            $this->relations=$this->guessRelations();
        }
	...
	public function generateRules($table)
	{
		...
		foreach($table->columns as $column)
		{
            ...
            else if(substr($column->dbType,0,4) === 'enum')
            {
                $enum[substr($column->dbType,4)][] = $column->name;
            }
			else if($column->type==='string' && $column->size>0)
				$length[$column->size][]=$column->name;
			else if(!$column->isPrimaryKey && !$r)
				$safe[]=$column->name;
            if(substr($column->name,-5) == 'image')
                $safe[]='delete_'.$column->name;
		}
		if($required!==array())
        ...
        if($enum !== array())
        {
            foreach($enum as $values=>$cols)
            {
                $rules[]="array('".implode(', ',$cols)."', 'in', 'range'=>array$values)";
            }
    ...
    protected function guessRelations()
    {
        if(!$this->guessRelations)
            return array();
        $relations=array();

        $schema = $this->getTableSchema($this->tableName);


        foreach($schema->columns as $item)
        {
            if($item->isPrimaryKey) continue;
            if(substr($item->name,0,3) == 'id_')
            {
                $model = $this->generateClassName(substr($item->name,3));

                $relations[$this->modelClass][substr($item->name,3)]="array(self::BELONGS_TO, '$model', '{$item->name}')";
